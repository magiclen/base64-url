...
/// Encode data to a Base64-URL string into a slice and return the slice with a valid length.
#[inline]
pub fn encode_in_place<'a, T: ?Sized + AsRef<[u8]>>(input: &T, output: &'a mut [u8]) -> &'a [u8] {
    let length = base64::encode_config_slice(input, base64::URL_SAFE_NO_PAD, output);

    &output[..length]
}

/// Encode data to a Base64-URL string and directly store into a mutable `String` reference by concatenating them and return the slice of the Base64-URL string. It is usually for generating a URL.
#[inline]
...
#[deprecated(since = "1.4.0", note = "Please use the `encode_in_place` function instead")]
...
pub fn encode_and_store_to_slice<T: ?Sized + AsRef<[u8]>>(input: &T, output: &mut [u8]) -> usize {
    encode_in_place(input, output).len()
}
